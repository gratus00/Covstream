...
structure WelfordCov (k : Nat) where
  ...
  mean              : Fin k -> Float  -- current running mean for each asset
  C        : Fin k -> Fin k -> Float  -- accumulator for covariance (not yet divided)
...
def WelfordCov.observe( est : WelfordCov k ) ( x: Fin k -> Float ) : WelfordCov k :=
  let n'    := est.n +1
...
theorem observe_increments_n {k : Nat} (est : WelfordCov k) (x : Fin k → Float) :
...
theorem observe_twice_n {k : Nat} (est : WelfordCov k) (x y : Fin k → Float) :
